...
function cargaDatosTabla(tabla){
    ...
    // Comprobamos que exista el elemento <table> antes de modificar su estilo
    const elementoTabla = document.querySelector("table");
    if(elementoTabla){
        elementoTabla.style.display = "table";
    } else {
        console.error("No se encontró el elemento <table> en el DOM.");
        return; // Salimos de la función si no existe la tabla
    }
        ...
        .then(datos => {
            ...
            columnas_tabla.forEach(function(columna, index){
                ...
                if(claves_tabla[index] != "MUL"){
                    ...
                    // Llamamos a la función para llenar las opciones del selector
                    ...
                    // Integramos la función para personalizar el select
