    ...
    if (isset($_GET['o'])) {
        switch ($_GET['o']) {
            ...
            case "tabla":
                ...
                // Instancia el codificador/decodificador
                $codificador = new Cifrado();

                // Itera sobre los datos y descifra los valores según corresponda
                foreach ($misdatos as &$fila) { // Usa referencia (&) para modificar directamente
                    foreach ($fila as $clave => &$valor) { // Usa referencia (&) para actualizar los valores
                        // Excluye la clave 'Identificador' y descifra solo valores no vacíos
                        if ($clave !== "Identificador" && !empty($valor)) {
                            $valor = $codificador->descodifica($valor);
                        }
                    }
                }
