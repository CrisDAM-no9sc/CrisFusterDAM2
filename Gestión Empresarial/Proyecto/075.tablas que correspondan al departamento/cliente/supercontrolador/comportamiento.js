...
let aplicacion = localStorage.getItem("crismon1_aplicacion");
    ...
    fetch("../../servidor/?o=listatablasaplicacion&aplicacion=" + aplicacion)
        .then(response => response.text()) // Utiliza .text() en lugar de .json() para verificar la respuesta cruda
        .then(datos => {
            console.log("Respuesta del servidor:", datos); // Mostrar lo que recibes
            try {
                // Intentamos parsear los datos a JSON si es posible
                let jsonData = JSON.parse(datos);
                if (Array.isArray(jsonData)) {
                    crearMenuTablas(jsonData, "tabla");
                } else {
                    console.error("Los datos no son un array como se esperaba:", jsonData);
                }
            } catch (e) {
                console.error("Error al parsear JSON:", e, datos);
            }
        })
        .catch(error => {
