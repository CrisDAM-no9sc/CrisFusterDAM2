...
def definir():
    ...
    ##traemos a la funcion la variable global
    global ruta_archivo_excel
    ##igualamos el valor de abirri archivo en tkinter
...
##hacemos un bucle for para asegursrnos de que pònemos tantas lineas que sean necesarias 
    for i in range(0,lineas):
        ...
        peticion = "INSERT INTO "+nombre_tabla.get()+" (Identificador) VALUES (NULL)" 
        print(peticion)
        cursor.execute(peticion)
        conexion.commit()
        ##que nos de el ultimo id que acabamos de meter
        peticion = "SELECT Identificador FROM "+nombre_tabla.get()+" ORDER BY Identificador DESC LIMIT 1"
